...
- [x] Non-Astra turns are verified live no-op. (PROVEN: Check A live turn with `gpt-5.6-sol`; targeted Astra event count remained unchanged at 18 -> 18; zero hook interference; repo clean.)
- [x] Plan mode, permissions, tools, and normal Codex workflows remain usable. (PROVEN: Check B live turn with `gpt-6-astra` under ordinary Workspace / Ask-for-approval permissions; 0 subagents; zero CAE interference; prompt completion reported PASS.)
...
- [x] Weekly window is shown when authoritatively exposed. (PROVEN: same; Task 2 weekly 1pt burn on stable epoch 1788793830; Check B weekly 1pt burn on stable epoch 1789254205.)
...
- [x] Receipts include Codex version and enough provenance to interpret the measurement. (PROVEN: Task 2 and Check B receipts record Codex 0.153.2, model, reasoning, authority, epochs, hook event times.)
...
- [x] Window 0 shakedown completed using only allowance already remaining before any banked reset. (PROVEN: 0 resets consumed, 2 remained; Window 0 CLOSED 2026-09-05.)
...
- [x] Final installed-artifact release-candidate validation. (PROVEN: Globally installed `codex-astra-efficiency@0.1.0` package verified end-to-end via Check A and Check B in `docs/V0_1_LIVE_RC_VALIDATION.md` and `receipts/v0.1-live-rc-validation-2026-09-05.md`.)
...
- [x] 5-hour and weekly effects are recorded separately. (PROVEN: all Window 0 and Check B receipts separate the windows.)
...
- [x] Local storage by default. (PROVEN: JSONL receipts under CAE state dir only; Task 2 and Check B privacy PASS.)
...
- [x] No quota bypass/reset/circumvention behavior. (PROVEN: no reset invocation in code; exactly one authorized reset consumed during live RC validation; second banked reset untouched.)
...
- **Gap B (Live non-Astra no-op):** PROVEN. Verified live in Check A with `gpt-5.6-sol` (0 new events recorded, targeted count unchanged at 18 -> 18).
...
- **Gap E (Normal permissions/workflow compatibility):** PROVEN. Verified live in Check B with `gpt-6-astra` under ordinary Workspace permissions.
...
2. Finalize v0.1.0 release candidate branch, run cross-platform CI, merge to main, and freeze `release/v0.1-candidate` (DONE);
3. Owner authorization on weekly allowance / banked reset (DONE: 1 reset consumed, second untouched);
4. Execute final installed-artifact live validation (`docs/V0_1_LIVE_RC_VALIDATION.md`) (DONE: PASS);
5. Final docs/evidence consistency sign-off cleanup (CURRENT);
6. Switch repository visibility to public;
7. Enable GitHub Private Vulnerability Reporting (`OWNER_UI_ACTION_AT_RELEASE`);
8. Tag `v0.1.0` and publish GitHub Release + npm registry package from tested artifact;
9. Begin post-release monitoring.
