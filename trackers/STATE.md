...
CAE is **live-Astra validated end-to-end** (Window 0 closed 2026-09-05): exact target, real hook capture, rootfs sandboxed writes, and the full setup/uninstall safety audit all pass on the authoritative runtime, with cross-platform CI green. Post-freeze pre-reset Astra audit has been recovered; the old candidate `2025274d51b082c0bdbb96a0d8106f3df28ac45b` was **SUSPENDED** due to the readiness live-capture P1 bug discovered in the audit. This P1 bug has been fully resolved and verified on branch `hardening/pre-window1-native-hook-readiness` (merged to main at `cd314fa4a2755c1ab1f79209243e9351bc859240`). A refreeze is complete; the new candidate `cd314fa4a2755c1ab1f79209243e9351bc859240` is **VALID** (pinned exactly on branch **`window1/candidate`**). Gaps P2/P3/P4 identified in the audit will form the post-reset hardening queue. Banked resets remain untouched at **2 remaining** and Window 1 has **not started**. Gaps A–F are explicit, support boundary declared (codexu authoritative; native Termux NOT claimed until Issue #9), and control starts at native-default `low`. Window 1 waits ONLY for an explicitly authorized banked reset (one planned; second untouched).
